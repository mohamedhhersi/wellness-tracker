...
const Hero: React.FC = () => {
  return (
    <motion.section
      className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-mice-elf-light to-mice-elf-dark text-white pt-24" // Added pt-24 for padding-top to prevent content from being hidden behind fixed navbar
      initial={{ opacity: 0, y: 0 }} // Initially, the Hero section is invisible (opacity: 0) and moved 50px down (y: 50)
      animate={{ opacity: 1, y: 0 }} // Animates to full opacity and moves up to its original position (y: 0)
      transition={{ duration: 1, delay: 0.1 }} // Animation lasts 1 second with a 0.5-second delay (starts after Navbar animation)
    >
